    ...
    <main className="min-h-screen bg-[#f4f5f7] grid-bg px-5 py-8">
      <div className="mx-auto max-w-6xl">
        <div className="app-gradient rounded-[2rem] p-7 text-white shadow-xl">
          ...
          <p className="mt-3 max-w-2xl text-blue-100">Choose a subject, then a chapter. Every chapter connects to official NCERT resources, your AI tutor and targeted practice.</p>
        </div>
          ...
          {subjects.map(subject => (
            <section key={subject.name} className="card rounded-2xl p-5">
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-50 text-xl font-black text-blue-600">{subject.icon}</div>
                <h2 className="text-xl font-black text-slate-900">{subject.name}</h2>
              </div>
              <div className="mt-5 space-y-2">
                {subject.chapters.map((chapter, index) => (
                  <Link key={chapter.title} href={`/library/topic?class=${encodeURIComponent(cls)}&subject=${encodeURIComponent(subject.name)}&chapter=${encodeURIComponent(chapter.title)}`} className="flex items-center justify-between rounded-xl border border-slate-100 bg-white px-3 py-3 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50">
                    <span><span className="mr-2 text-xs text-slate-400">{String(index + 1).padStart(2, '0')}</span>{chapter.title}</span><ChevronRight size={16} className="text-slate-400" />
                  </Link>
                ))}
              </div>
            </section>
