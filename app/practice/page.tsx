...
export default function PracticePage() {
  ...
  async function generate() {
    ...
    try {
      const r = await fetch('/api/practice', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({classNo,subject,topic,track,level,count:10}) });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'Unable to generate questions');
      setQuestions(d.questions || []);
    } catch (e:any) { setError(e.message || 'Unable to generate questions'); }
